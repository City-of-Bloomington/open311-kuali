...
public class BusServiceImpl implements BusService {
	...
	public List<BusRoute> getRoutes(String campus) {
		if (getDao().getBusRoutes() == null
				|| getDao().getBusRoutes().size() == 0) {
			getDao().loadRoutes();
		}
	...
	public List<BusStop> getStops(String campus) {
		if (getDao().getBusStops() == null
				|| getDao().getBusStops().size() == 0) {
			getDao().loadRoutes();
		}
	...
	public List<BusStop> getNearbyStops(double lat1, double lon1, double radius) {
		return getDao().findNearByStops(lat1, lon1, radius);
	}
