    ...
    <kme:content>
		...
		<script type="text/javascript">
			$('[data-role=page][id=computerlabs]').live('pagebeforeshow', function(event, ui) {
				...
				refreshTemplate('computerlabs/feed', '#computerlablist', 'clListTemplate', '<li>No labs were found</li>', function() {$('#computerlablist').listview('refresh');});
			});
		</script>
	            ...
	            <c:forEach items="${location.computerLabs}" var="computerlab" varStatus="status">
	                <kme:listItem cssClass="link-gps">
				       <c:choose>
			                ...
			                <c:otherwise>
                                <h3>HMTL: ${computerlab.labCode}</h3><p>${computerlab.availability} seats available</p>
			                </c:otherwise>
			            </c:choose>
	                </kme:listItem>
	            </c:forEach>
